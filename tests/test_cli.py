...
def test_check_passes_external_flag_to_service(plugin_factory, monkeypatch) -> None:
    root = plugin_factory()
    captured: dict[str, object] = {}

    def fake_run_check(path, selector, dry_run, external=False):
        captured.update(path=path, selector=selector, dry_run=dry_run, external=external)
    ...
    result = CliRunner().invoke(app, ["check", str(root), "--external", "--dry-run"])
